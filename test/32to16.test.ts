...
import { getFloat16Precision, setFloat16, getFloat16, format32, Precision } from "fp16"
...
test("convert 32-bit values to 16-bit values (big-endian)", (t) => {
	for (const [i32, i16] of conversions) {
		float32View.setInt32(0, i32)
		const f = float32View.getFloat32(0)
		setFloat16(float16View, 0, f)
		t.is(float16View.getUint16(0), i16, `(${format32(i32)}) => 0x${i16.toString(16).padStart(4, "0")}`)
	}
})

test("convert 32-bit values to 16-bit values (little-endian)", (t) => {
	for (const [i32, i16] of conversions) {
		float32View.setInt32(0, i32, true)
		const f = float32View.getFloat32(0, true)
		setFloat16(float16View, 0, f, true)
		t.is(float16View.getUint16(0, true), i16, `(${format32(i32)}) => 0x${i16.toString(16).padStart(4, "0")}`)
	}
})
